...
use ghostengine::{Page, render, anti};
...
fn main() {
    ...
    for w in &v.walls {
        println!("  tech={} sitekey={:?} keyless={} hint={}",
                 w.tech.name(), w.sitekey.as_deref().map(|s| &s[..s.len().min(14)]),
                 w.tech.keyless(), &w.hint[..w.hint.len().min(40)]);
    }
    // clean page control
    let out2 = std::process::Command::new("../target/release/ghostmouse")
        .args(["get", "https://httpbin.org/html"])
        .output().expect("fetch2");
    let clean = Page::parse(&String::from_utf8_lossy(&out2.stdout));
    let v2 = anti::verdict(&clean);
    println!("\nclean-page control: walled={} (false hona chahiye)", v2.walled);
}
