//! GhostMouse v0.3 — stealth agentic web client for AI agents.
//!
...
mod search {
    ...
    /// then Bing HTML (heavier, different index). First engine returning
    /// results wins; a captcha-walled engine degrades to the next.
    pub async fn run(
        agent: &mut Agent,
        query: &str,
        limit: usize,
...
mod agent {
    ...
    impl Agent {
        pub fn new(user: &str, opts: &AgentOpts) -> Result<Self, String> {
            ...
            // navigation headers ride on top of that.
        ...
        /// X/Twitter posts without login, same as the Python netkit.
...
mod captcha {
    ...
    /// What kind of challenge a page carries (live-tested markers,
    /// kept in sync with recon/cf_probe.py and solver/server.py probe).
    pub fn detect(page_html: &str) -> Vec<&'static str> {
        let mut found = Vec::new();
        if page_html.contains("challenges.cloudflare.com/turnstile") {
            found.push("cloudflare-turnstile");
        }
        if page_html.contains("hcaptcha.com")
            || page_html.contains("h-captcha")
            || page_html.contains("hcaptcha_token")
        {
            found.push("hcaptcha");
        }
        if page_html.contains("google.com/recaptcha") || page_html.contains("g-recaptcha") {
            found.push("recaptcha");
        }
        if page_html.contains("Just a moment") || page_html.contains("cf-chl") {
    ...
    pub fn sitekeys(page_html: &str) -> Vec<String> {
        ...
        while let Some(i) = r.find(|c: char| c.is_ascii_uppercase() || c == '_') {
            r = &r[i..];
            let window: &str = &r[..r.len().min(400)];
            ...
            Cmd::Whoami => {
                println!("{}", serde_json::to_string_pretty(&json!({
                    ...
                    "features": ["chrome-tls", "tracker-block", "vault-clearance",
                                ...
                                "captcha-solve", "x-syndication", "jsonl-drive-api"],
                })).unwrap());
            }
        }
        Ok(())
